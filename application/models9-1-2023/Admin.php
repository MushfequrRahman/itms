...
class Admin extends CI_Model
{
	...
	// 		return $query=$this->db->query($sql);
	// 	}
	public function receive_create($userid,$mprid,$item,$sipoid,$po,$grn,$rqty,$rdate,$rremarks)
	{
		date_default_timezone_set('Asia/Dhaka');
		$rdate = date("Y-m-d", strtotime($rdate));
		$d = date('Y-m-d');
		...
		//if($data['pstatus']!=3)
		//		{
		$sql = "INSERT INTO receive_insert VALUES ('$ccid','$ccid1','$userid','$mprid','$item','$sipoid','$po','$grn','$rqty','$rremarks','$rdate')";
		//}
		//else
		//		{
		//			$sql="INSERT INTO po_insert VALUES ('$ccid','$ccid1','$data[userid]','$data[mprid]','','$data[item]','$data[pqty]','$data[premarks]','','','','$data[pstatus]')";
