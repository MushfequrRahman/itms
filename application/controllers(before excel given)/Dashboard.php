		...
		if($this->input->post('submit'))
		{
			...
			if($this->form_validation->run()==FALSE)
			...
			else
				{
					...
					$ins=$this->Admin->product_details_insert($pccode,$factoryid,$mpr,$brand,$hdd,$monitor,$ups,$ip,$mb,$sn,$processor,$ram,$os,$ms,$avid,$iid,$description,$price,$pqty,$puomid,$vendor,$pcapop,$warranty,$pdate);
	...
	public function productdetailslup()
	 ...
	 public function producthistorylup()
	 {
		...
		if($this->input->post('submit'))
		{
			...
			$pcode=$this->input->post('pcode');
			$factoryid=$this->input->post('factoryid');
			$ins=$this->Admin->producthistorylup($factoryid,$pcode);
			if($ins==TRUE)
				{
					$this->session->set_flashdata('Successfully','Successfully Updated');
				}
