...
//		$data['il']=$this->Admin->internet();
	...
	public function product_dispose_form()
	 {
		$this->load->database();
		$this->load->model('Admin');
		$data['title']='Product Dispose';
		$pacode= $this->uri->segment(3);
		$this->load->view('admin/head',$data);
		$this->load->view('admin/toprightnav');
		$this->load->view('admin/leftmenu');
		$data['pacode']=$pacode;
		$this->load->view('admin/product_dispose_form',$data);
	 }
	 public function product_dispose_insert()
	 {
		$this->load->database();
		$this->load->library('form_validation');
		$this->load->model('Admin');
		if($this->input->post('submit'))
		{
			...
			if($this->form_validation->run()==FALSE)
			...
			else
				{
					...
					$ins=$this->Admin->product_dispose_insert($pacode,$remarks,$ddate);
					...
					if($ins==TRUE)
						...
						{
							$this->session->set_flashdata('Successfully','Failed To Disposed');
						}
					redirect('Dashboard/product_details_list','refresh');
				}
		}
	...
	public function product_inventory_insert_form()
	 {
		...
		$this->load->view('admin/toprightnav');
		$this->load->view('admin/leftmenu');
		$data['ul']=$this->Admin->product_inventory_view($pcode);
		$data['ul2']=$this->Admin->product_inventory_view1($pcode);
		$data['ul1']=$this->Admin->receive_qty($pcode);
		$data['fl']=$this->Admin->factory_list();
		$data['pcode']=$pcode;
		...
		if($this->input->post('submit'))
		{
			...
			if($this->form_validation->run()==FALSE)
			...
			else
				{
					...
					$iqty=$this->input->post('iqty');
					$warranty=$this->input->post('warranty');
					$ins=$this->Admin->product_inventory_insert($pcode,$pccode,$factoryid,$sn,$description,$pdate,$iqty,$warranty);
