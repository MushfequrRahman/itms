          ...
          <div class="row">
            ...
            <div class="box-body">
				...
				{ ?>
                  ...
                  <form role="form" autocomplete="off" action="<?php echo base_url();?>Dashboard/suggession_inquiry_insert" method="post" enctype="multipart/form-data">
                ...
                <div class="form-group">
					<label>Corrective Message<em>*</em></label>
                 	<textarea class="form-control" name="corrective" rows="5"></textarea>
                  </div> 
                <div class="form-group">
					<label>Grading<em>*</em></label>
					<select class="form-control" name="graid" id="graid">
                    	<option value="">Select....</option>
                        <?php
						foreach($gl as $row)
						{
					?>
                    		<option value="<?php echo $row['graid'];?>"><?php echo $row['grading'];?></option>
                    <?php
						}
					?>
                    </select>
                    <?php echo form_error('graid', '<div class="error">', '</div>');  ?>
				</div> 
                <div class="form-group">
					<label>FPR<em>*</em></label>
					<select class="form-control" name="fprid" id="fprid">
                    	<option value="">Select....</option>
                        <?php
						foreach($fprl as $row)
						{
					?>
                    		<option value="<?php echo $row['fprid'];?>"><?php echo $row['ename'];?></option>
                    <?php
						}
					?>
                    </select>
                    <?php echo form_error('fprid', '<div class="error">', '</div>');  ?>
				</div>
                <div class="form-group">
					<label>Inquiry Day<em>*</em></label>
                 	<input type="text" class="form-control" name="inqday" placeholde="Enter Inquiry Day">
                  </div>
