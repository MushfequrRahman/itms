...
class Admin extends CI_Model
{
	...
	public function product_details_insert($pcode, $factoryid, $supplierid, $mpr, $sn, $description, $pqty, $puomid, $warranty, $pr, $pdate)
	{
		...
		$sql = "INSERT INTO product_details_insert VALUES ('$pacode','$pcode','$factoryid','$supplierid','$mpr','$sn','$description','$pqty','$puomid','$warranty','$pr','$pdate','','','','0')";
		$query = $this->db->query($sql);
	...
	//	{
	//		$query = "SELECT pdiid,product_category_insert.pccode,product_details_insert.pcode,pcname,mpr,brand,hdd,monitor,ups,ip,mb,sn,processor,ram,os,ms,avid,iid,description,price,pqty,puom,vendor,warranty,pdate,pastatus,adate,product_details_insert.userid,name,image 
	...
	// public function product_details_list()
	// {
	// 	$query = "SELECT product_details_insert.pdiid,product_category_insert.pccode,product_details_insert.pcode,pcname,product_history_insert.factoryid,
	// 	supplier,pgname,psgname,mpr,sn,description,pqty,puom,warranty,pdate,pastatus,adate,product_details_insert.userid,name,image 
	// 	FROM product_details_insert
	// 	LEFT JOIN product_category_insert ON product_category_insert.pccode=product_details_insert.pccode
	// 	LEFT JOIN product_uom_insert ON product_uom_insert.puomid=product_details_insert.puomid
	// 	JOIN  product_history_insert ON product_history_insert.pdiid=product_details_insert.pdiid
	// 	JOIN  supplier_insert ON supplier_insert.supplierid=product_details_insert.supplierid
	// 	JOIN  product_group_insert ON product_group_insert.pgid=product_details_insert.pgid
	// 	JOIN  product_subgroup_insert ON product_subgroup_insert.psgid=product_details_insert.psgid
	// 	LEFT JOIN user ON user.userid=product_details_insert.userid
	...
	public function product_details_list()
	{
		...
		mpr,sn,description,pqty,puom,warranty,price,pdate,adate,rdate,pcname,
	...
	public function product_details_list_up($pacode)
	{
		...
		JOIN product_subgroup_insert ON product_subgroup_insert.psgid=product_insert.psgid
		JOIN product_history_insert ON product_history_insert.pacode=product_details_insert.pacode
		WHERE product_history_insert.pacode='$pacode' AND product_history_insert.phstatus='1'";
		$query1 = $this->db->query($sql1);
		$result = $query1->result_array();
		return $result;
	}
	...
	public function producthistorylup($factoryid, $pacode)
	{
		...
		//$sql1 = "INSERT INTO product_ihistory_insert VALUES ('','$pcode','$factoryid','1')";
		//		$query1 = $this->db->query($sql1);
		//		return $query1;
	}
	public function product_inventory_list_up($pdiid)
	{
		$sql = "SELECT * FROM product_inventory WHERE piv='$pdiid'";
		$query = $this->db->query($sql);
	...
	public function product_inventory_view($pcode)
	{
		$querytt = "SELECT sipoid,fid,pccode FROM mpr_wise_receive_list_view WHERE sipoid='$pcode'";
	...
	public function product_inventory_view1($pcode)
	{
		$querytt = "SELECT fid,pccode FROM mpr_wise_receive_list_view WHERE sipoid='$pcode'";
		$resulttt = $this->db->query($querytt);
		return $resulttt = $resulttt->result_array();
	}
	public function receive_qty($pcode)
	{

		$query = "SELECT SUM(rqty) AS rqty FROM receive_insert
				  WHERE sipoid='$pcode' GROUP BY sipoid";
		$result = $this->db->query($query);
		return $result->result_array();
	}
	...
	//		return $result->result_array();
	//	}
	public function product_inventory_insert($pcode, $pccode, $factoryid, $sn, $description, $pdate, $iqty, $warranty)
	{
		date_default_timezone_set('Asia/Dhaka');
		$pdate = date("Y-m-d", strtotime($pdate));
		$d = date('Y-m-d');
		...
		$sql1a = "SELECT MAX(pcidnum) AS pcidnum FROM product_asset_code WHERE pcid='$pccode' ";
		$query1a = $this->db->query($sql1a);
		$result1a = $query1a->result_array();
		foreach ($result1a as $row) {
			$pcidnum = $row['pcidnum'];
		...
		$pacode = $factoryid . '-' . $pccode . '-' . $pcidnum;
		$sqla = "INSERT INTO product_asset_code VALUES ('$ccid','$factoryid','$pccode','$pacode','$pcidnum')";
		$sql = "INSERT INTO product_inventory VALUES ('$ccid','$pcode','$factoryid','$pacode','$sn','$description','$iqty','$warranty','$pdate','','','','0')";

		$sqll = "INSERT INTO product_ihistory_insert VALUES ('','$pcode','$factoryid','1')";
