                  ...
                  <tbody>
                    ...
                    <tr id="trid<?php echo $i; ?>">
                      ...
                      ?>
                        ...
                        <td style="vertical-align:middle; width: 5%;"><input type="number" min="1" class="form-control" style="min-width:80px;" id="rqty<?php echo $i; ?>" name="rqty" placeholder="Qty">
                        </td>
...
<script>
  $(document).ready(function() {
    ...
    // সর্বোচ্চ যত row থাকতে পারে (আপনার দরকার অনুযায়ী বাড়াতে পারেন)
    for (let i = 1; i <= 50; i++) {
      ...
      $(document).on('click', '#btn' + i, function(event) {
        ...
        let item = $('#item' + i).val();
        let sipoid = $('#sipoid' + i).val();
        let po = $('#po' + i).val();
        let grn = $('#grn' + i).val();

        let rqty = parseFloat($('#rqty' + i).val());
        let reqty = parseFloat($('#reqty' + i).val());

        let rremarks = $('#rremarks' + i).val();
        let rdate = $('#rdate' + i).val();
        let invoice = $('#invoice' + i).val();
        let cdate = $('#cdate' + i).val();
        ...
        if (isNaN(rqty) || rqty <= 0) {
          error += "Receive Qty must be greater than 0<br>";
        }

        if (!isNaN(rqty) && rqty > reqty) {
          error += "Receive Qty must be less than or equal to Remaining Qty<br>";
        }

        if (grn === '') {
          error += "GRN is required<br>";
        }
        ...
        $.ajax({
          ...
          success: function(response) {
            ...
            // current row hide
            $('#trid' + i).delay(500).fadeOut();

            $('#error').html(
              '<div class="alert alert-success">Receive saved successfully</div>'
            ).delay(2000).fadeOut();
          },
          error: function() {
            $('#error').html(
