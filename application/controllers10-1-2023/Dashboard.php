	...
	public function productdetailslup()
	 ...
	 public function productinventorylup()
	 {
		...
		if($this->input->post('submit'))
		{
			...
			$ins=$this->Admin->productinventorylup($piv,$factoryid,$sn,$description,$iqty,$warranty,$pdate);
			if($ins==TRUE)
				{
					$this->session->set_flashdata('Successfully','Successfully Updated');
				}
		...
		if($this->input->post('submit'))
		{
			...
			if($this->form_validation->run()==FALSE)
			...
			else
				{
					...
					$iqty=$this->input->post('iqty');
					$warranty=$this->input->post('warranty');
					$ins=$this->Admin->product_inventory_insert($pcode,$factoryid,$sn,$description,$pdate,$iqty,$warranty);
