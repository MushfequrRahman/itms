...
//            }else{
	 ...
	 public function ulup()
	 {
		...
		if($this->input->post('submit'))
		{
					...
					//Active OR Inactive
					if($userstatusid==1 || $userstatusid==2 || $userstatusid==4)
					{
...
//		$this->load->view('admin/leftmenu');
	...
	public function vehicle_list()
	 ...
	 public function addvreading_insert_form()
	 {
		$this->load->database();
		$this->load->model('Admin');
		$data['title']='ADD Vehicle Reading Info';
		$this->load->view('admin/head',$data);
		$this->load->view('admin/toprightnav');
		$this->load->view('admin/leftmenu');
		$this->load->view('admin/addvreading_insert_form',$data);
	}
	
	public function addvreading_insert()
	 {
		$this->load->database();
		$this->load->library('form_validation');
		$this->load->model('Admin');
		if($this->input->post('submit'))
		{
			$vrn=$this->form_validation->set_rules('vrn', 'Vehicle Register Number', 'required');
			$msr=$this->form_validation->set_rules('msr', 'Month Start Reading', 'required');
			$mer=$this->form_validation->set_rules('mer', 'Month End Reading', 'required');
			//$mdate=$this->form_validation->set_rules('mdate', 'Date', 'required');
			if($this->form_validation->run()==FALSE)
				{
					$this->addvreading_insert_form();
				}
			else
				{
					$vrn=$this->input->post('vrn');
					$msr=$this->input->post('msr');
					$mer=$this->input->post('mer');
					$mdate=$this->input->post('mdate');
					$ins=$this->Admin->addvreading_insert($vrn,$msr,$mer,$mdate);
					...
					redirect('Dashboard/addvreading_insert_form','refresh');
				}
		}
	}
	public function addvfueluse_insert_form()
	 {
		$this->load->database();
		$this->load->model('Admin');
		$data['title']='ADD Vehicle Fuel Use Info';
		$this->load->view('admin/head',$data);
		$this->load->view('admin/toprightnav');
		$this->load->view('admin/leftmenu');
		$data['vful']=$this->Admin->vfuel_list();
		$this->load->view('admin/addvfueluse_insert_form',$data);
	}
	public function addvfueluse_insert()
	 {
		$this->load->database();
		$this->load->library('form_validation');
		$this->load->model('Admin');
		if($this->input->post('submit'))
		{
			$vrn=$this->form_validation->set_rules('vrn', 'Vehicle Register Number', 'required');
			$fuel=$this->form_validation->set_rules('fuel', 'Fuel', 'required');
			$vfqty=$this->form_validation->set_rules('vfqty', 'Fuel Use Qty', 'required');
			if($this->form_validation->run()==FALSE)
				{
					$this->addvfueluse_insert_form();
				}
			else
				{
					$vrn=$this->input->post('vrn');
					$fuel=$this->input->post('fuel');
					$vfqty=$this->input->post('vfqty');
					$mdate=$this->input->post('mdate');
					$ins=$this->Admin->addvfueluse_insert($vrn,$fuel,$vfqty,$mdate);
					...
					redirect('Dashboard/addvfueluse_insert_form','refresh');
				}
		}
	}
	public function vreading_list_form()
	 {
		$this->load->database();
		$this->load->model('Admin');
		$data['title']='Vehicle Meter Reading Info';
		$this->load->view('admin/head',$data);
		$this->load->view('admin/toprightnav');
		$this->load->view('admin/leftmenu');
		$data['ulm']=$this->Admin->vfuelusem_list();
		$data['uly']=$this->Admin->vfuelusey_list();
		$this->load->view('admin/vreading_list_form',$data);
	}
	public function vreading_list()
	 {
		$this->load->database();
		$this->load->model('Admin');
		$data['title']='Vehicle Meter Reading Info';
		$mo=$this->input->post('mo');
		$yr=$this->input->post('yr');
		$data['ul']=$this->Admin->vreading_list($mo,$yr);
		$this->load->view('admin/vreading_list',$data);
	 }
	 public function vfuse_list_form()
	 {
		$this->load->database();
		$this->load->model('Admin');
		$data['title']='Vehicle Fuel Use Info';
		$this->load->view('admin/head',$data);
		$this->load->view('admin/toprightnav');
		$this->load->view('admin/leftmenu');
		$this->load->view('admin/head',$data);
		...
		$this->load->view('admin/leftmenu');
		//$data['vful']=$this->Admin->vfuel_list();
		$this->load->view('admin/date_wise_vfuse_list_form',$data);
	}
	public function date_wise_vfuse_list()
	 {
		$this->load->database();
		$this->load->model('Admin');
		$data['title']='Vehicle Fuel Use Info';
		$pd=$this->input->post('pd');
		$wd=$this->input->post('wd');
		$data['ul']=$this->Admin->date_wise_vfuse_list($pd,$wd);
		$this->load->view('admin/date_wise_vfuse_list',$data);
	 }
	 public function mvfuse_list_form()
	 {
		$this->load->database();
		$this->load->model('Admin');
		$data['title']='Vehicle Fuel Use Info';
		...
		$data['ulm']=$this->Admin->vfuelusem_list();
		$data['uly']=$this->Admin->vfuelusey_list();
		$this->load->view('admin/mvmfuse_list_form',$data);
	}
	public function mvmfuse_list()
	 {
		$this->load->database();
		$this->load->model('Admin');
		$data['title']='Vehicle Fuel Use Info';
		$mo=$this->input->post('mo');
		$yr=$this->input->post('yr');
		$query=$this->db->query("SELECT vrn,(vr/vfuse) AS ku FROM vreading_list_view
				...
				LEFT JOIN vfuel_insert ON vfuel_insert.vfuid=vfueluse_total_view.vfuelid
		 		WHERE vmonth='$mo' AND vyear='$yr'");
		      	$record = $query->result();
				$data = array();
				foreach($record as $row) 
					{
						$data['label'][] = $row->vrn;
						$data['data'][] = (int) $row->ku;
					}
				$data['chart_data'] = json_encode($data);
				$data['ul']=$this->Admin->mvmfuse_list($mo,$yr);
				$this->load->view('admin/mvmfuse_list',$data);
	 }
