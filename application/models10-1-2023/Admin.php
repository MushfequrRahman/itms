...
//	}
	...
	public function productinventorylup($piv,$factoryid,$sn,$description,$iqty,$warranty,$pdate)
	{
		$pdate = date("Y-m-d", strtotime($pdate));
		$sql = "UPDATE product_inventory SET 	 		ifid='$factoryid',sn='$sn',idescription='$description',iqty='$iqty',warranty='$warranty',pdate='$pdate' 
		WHERE piv='$piv'";
		return $query = $this->db->query($sql);
	}
...
//		WHERE receive_insert.mprid='$mprid' AND grn!=''";
	...
	{
		$querytt="SELECT sipoid FROM mpr_wise_receive_list_view WHERE sipoid='$pcode'";
...
//		return $result->result_array();
//	}
	public function product_inventory_insert($pcode,$sn,$description,$pdate,$iqty,$warranty)
	{
		date_default_timezone_set('Asia/Dhaka');
		$pdate = date("Y-m-d", strtotime($pdate));
		$d = date('Y-m-d');
		...
		$t1 = str_replace(":", "", $t);
		$ccid = $d1 . $t1;
		$sql = "INSERT INTO product_inventory VALUES ('$ccid','$pcode','$factoryid','$sn','$description','$iqty','$warranty','$pdate','','','','0')";
		return $query = $this->db->query($sql);
	}
