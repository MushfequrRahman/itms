...
class Dashboard extends CI_Controller
{
	...
	public function index()
	{
		...
		// === (1) Current Month MPR ===
		$query_mpr = $this->db->query("
    SELECT fid, 
           COUNT(DISTINCT mprid) AS total_mpr,
           SUM(price) AS total_mpr_value
    FROM mpr_insert_id 
    ...
    GROUP BY fid
");
		$record_mpr = $query_mpr->result();

		// === (2) Current Month PO ===
		$query_po = $this->db->query("
    SELECT mpr_insert_id.fid,
           COUNT(DISTINCT po_insert.po) AS total_po,
           SUM(po_insert.pprice * po_insert.pqty) AS total_po_value
    FROM mpr_insert_id
    JOIN po_insert ON po_insert.mprid = mpr_insert_id.mprid
    WHERE MONTH(po_insert.pdate) = MONTH(NOW())
      AND YEAR(po_insert.pdate) = YEAR(NOW())
    GROUP BY mpr_insert_id.fid
");
		$record_po = $query_po->result();

		// === (3) Old MPR → This Month PO ===
		$query_old_mpr_po = $this->db->query("
    SELECT mpr_insert_id.fid,
           COUNT(DISTINCT po_insert.po) AS old_po_count,
           SUM(po_insert.pprice * po_insert.pqty) AS old_po_value,
           COUNT(DISTINCT po_insert.mprid) AS old_mpr_count
    FROM mpr_insert_id
    JOIN po_insert ON po_insert.mprid = mpr_insert_id.mprid
    WHERE MONTH(po_insert.pdate) = MONTH(NOW())
      AND YEAR(po_insert.pdate) = YEAR(NOW())
      AND NOT (MONTH(mpr_insert_id.mdate) = MONTH(NOW()) 
               AND YEAR(mpr_insert_id.mdate) = YEAR(NOW()))
    GROUP BY mpr_insert_id.fid
");
		$record_old_mpr_po = $query_old_mpr_po->result();
		...
		// Step 1: Current Month PO
		foreach ($record_po as $po) {
			$fid = $po->fid;
			$data1['label'][] = $fid;
			$data1['po_value'][] = (int)$po->total_po_value;
			$data1['po_count'][] = (int)$po->total_po;
			$data1['mpr_count'][] = 0;
			$data1['mpr_value'][] = 0;
			$data1['old_po_value'][] = 0;
			$data1['old_po_count'][] = 0;
			$data1['old_mpr_count'][] = 0;
		}

		// Step 2: Merge MPR Data
		foreach ($record_mpr as $mpr) {
			$fid = $mpr->fid;
			$key = array_search($fid, $data1['label']);
			if ($key !== false) {
				$data1['mpr_count'][$key] = (int)$mpr->total_mpr;
				$data1['mpr_value'][$key] = (int)$mpr->total_mpr_value;
			} else {
				$data1['label'][] = $fid;
				$data1['po_value'][] = 0;
				$data1['po_count'][] = 0;
				$data1['mpr_count'][] = (int)$mpr->total_mpr;
				$data1['mpr_value'][] = (int)$mpr->total_mpr_value;
				$data1['old_po_value'][] = 0;
				$data1['old_po_count'][] = 0;
				$data1['old_mpr_count'][] = 0;
			}
		...
		// Step 3: Merge Old MPR → This Month PO
		foreach ($record_old_mpr_po as $old) {
			$fid = $old->fid;
			$key = array_search($fid, $data1['label']);
			if ($key !== false) {
				$data1['old_po_value'][$key] = (int)$old->old_po_value;
				$data1['old_po_count'][$key] = (int)$old->old_po_count;
				$data1['old_mpr_count'][$key] = (int)$old->old_mpr_count;
			} else {
				$data1['label'][] = $fid;
				$data1['po_value'][] = 0;
				$data1['po_count'][] = 0;
				$data1['mpr_count'][] = 0;
				$data1['mpr_value'][] = 0;
				$data1['old_po_value'][] = (int)$old->old_po_value;
				$data1['old_po_count'][] = (int)$old->old_po_count;
				$data1['old_mpr_count'][] = (int)$old->old_mpr_count;
			}
		}
...
");
	...
	public function mpr_create()
	{
		...
		while (true) {
			$sql = "SELECT * FROM mpr_insert_id WHERE smprid='$ccid'";
			$query = $this->db->query($sql);
		...
		// এখন চেক করব $mprid আগে থেকেই আছে কিনা
		$sql_check_mprid = "SELECT * FROM mpr_insert_id WHERE mprid='$mprid'";
		$query_check_mprid = $this->db->query($sql_check_mprid);

		if ($query_check_mprid->num_rows() > 0) {
			// যদি mprid আগেই থাকে, তাহলে Error দেখাও বা Update করো
			echo "duplicate_mprid";
			return;
		}
