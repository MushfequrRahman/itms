...
INSERT INTO `mpr_insert` (`smprid`, `simprid`, `suserid`, `mprid`, `fid`, `mdeptid`, `name`, `mdesigid`, `item`, `type`, `qty`, `uom`, `description`, `price`, `remarks`, `uname`, `mdate`, `mstatus`) VALUES
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `po_qty_remaining`  AS  select `mpr_insert`.`mprid` AS `mprid`,`mpr_insert`.`fid` AS `fid`,`product_category_insert`.`pcname` AS `pcname`,`po_insert`.`simprid` AS `simprid`,`product_capop_insert`.`pcapop` AS `pcapop`,`mpr_insert`.`item` AS `item`,`mpr_insert`.`qty` AS `qty`,`product_uom_insert`.`puom` AS `puom`,sum(`po_insert`.`pqty`) AS `prqty`,sum(`po_insert`.`pprice`) AS `tpprice` from ((((((`po_insert` join `mpr_insert` on(`mpr_insert`.`simprid` = `po_insert`.`simprid`)) join `product_uom_insert` on(`product_uom_insert`.`puomid` = `mpr_insert`.`uom`)) join `product_category_insert` on(`product_category_insert`.`pccode` = `mpr_insert`.`item`)) join `product_capop_insert` on(`product_capop_insert`.`pcoid` = `mpr_insert`.`type`)) join `department` on(`department`.`deptid` = `mpr_insert`.`mdeptid`)) join `designation` on(`designation`.`desigid` = `mpr_insert`.`mdesigid`)) group by `po_insert`.`simprid` ;
